...
public class UIPlatformButton : UIButtonColor
{
	/// <summary>
	/// Current button that sent out the onClick event.
	/// </summary>

    static public UIPlatformButton current;
    ...
    public List<EventDelegate> onClickAndroid = new List<EventDelegate>();
    public List<EventDelegate> onClickIOS = new List<EventDelegate>();
    public List<EventDelegate> onClickWebPlayer = new List<EventDelegate>();
    public List<EventDelegate> onClickOther = new List<EventDelegate>();
	...
	public override bool isEnabled
	{
		...
		set
		{
			if (isEnabled != value)
			{
				...
				{
					col.enabled = value;
                    UIPlatformButton[] buttons = GetComponents<UIPlatformButton>();
                    foreach (UIPlatformButton btn in buttons) btn.SetState(value ? State.Normal : State.Disabled, false);
				}
				else
				{
					Collider2D c2d = GetComponent<Collider2D>();
...
#if UNITY_IOS
			EventDelegate.Execute(onClickIOS);
#elif UNITY_ANDROID
            EventDelegate.Execute(onClickAndroid);
#elif UNITY_WEBPLAYER
			EventDelegate.Execute(onClickWebPlayer);
#elif !UNITY_WEBPLAYER && !UNITY_ANDROID && !UNITY_IOS
			EventDelegate.Execute(onClickOther);
#endif
