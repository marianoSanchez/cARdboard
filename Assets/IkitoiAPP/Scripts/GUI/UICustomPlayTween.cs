...
[ExecuteInEditMode]
public class UICustomPlayTween : MonoBehaviour
{
    static public UICustomPlayTween current;
	...
	public void Play (bool forward)
	{
        ...
        mTweens = includeChildren ? GetComponentsInFirstChildrens<UITweener>(go).ToArray() : go.GetComponents<UITweener>();
    ...
    List<T> GetComponentsInFirstChildrens<T>(GameObject gameObject)
    {
        var components = new List<T>();
        for (int i = 0; i < gameObject.transform.childCount; i++)
        {
            var child = gameObject.transform.GetChild(i);
            var cComp = child.GetComponents<T>();
            if (cComp != null && cComp.Length > 0)
                components.AddRange(cComp);
        }
