...
[ExecuteInEditMode]
public class UICustomDragObject : MonoBehaviour
{
	...
	public Vector3 dragMovement { get { return dragScale; } set { dragScale = value; } }
    ...
    [SerializeField]
    protected Vector3 dragScale = new Vector3(1f, 1f, 0f);
	...
	Bounds mBounds;
	bool mStarted = false;
    List<int> mTouchsID = new List<int>();

    public ReadOnlyCollection<int> TouchsID
    {
        get { return mTouchsID.AsReadOnly(); }
    }

    /// <summary>
    /// Object is being pressed?
    /// </summary>
    /// <returns>Object is being pressed?</returns>

    public bool IsPressed()
    {
        return mTouchsID.Count > 0;
    }

    /// <summary>
    /// Object is being touch by exactly a number of touchs
    /// </summary>
    /// <param name="touchs">Number of touchs</param>
    /// <returns>Object is being touch by exactly a number of touchs</returns>
    
    public bool IsPressed(int touchs)
    {
        return mTouchsID.Count == touchs;
    }
	...
	/// <summary>
	/// Recalculate the bounds of the dragged / zoomed content.
	/// </summary>
	...
	void OnPress (bool pressed)
    {
        ...
        // Object and component enabled
		if (enabled && NGUITools.GetActive(gameObject) && target != null)
		{
            // New Press
			if (pressed)
			{
                // If the object was not being touched
                if (!IsPressed())
				{
                ...
                // The object was already being touch
                else if (IsPressed(1))
                {
                    // Remove all momentum
                    mStarted = false;
                ...
                // Add the new touch
                mTouchsID.Add(UICamera.currentTouchID);
			}
            ...
            else
			{
                ...
                // Removes the touch form the list
                mTouchsID.Remove(UICamera.currentTouchID);
			}
		}
	}
	...
	{
        // Is being dragged by one touch
        if (IsPressed(1))
        {
            if (mTouchsID[0] == UICamera.currentTouchID && enabled && NGUITools.GetActive(gameObject) && target != null)
            {
