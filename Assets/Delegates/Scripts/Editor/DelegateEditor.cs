...
public static class EditorDelegateGUI
{
	public static void EventFields (Component targetBehavior)
	{
		...
		{			
			//
			// GET/SET FIELD
			if(field.FieldType == typeof(DelegateMessage[]) || field.FieldType == typeof(SerializableDelegate))
			{
				//
				// ARRAY FIELDS
				DelegateMessage[] oldEvents = field.GetValue(targetBehavior) as DelegateMessage[];
				if(field.FieldType == typeof(SerializableDelegate))
				{
					SerializableDelegate list = (SerializableDelegate)field.GetValue(targetBehavior);
